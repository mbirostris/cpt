...
class synchronizer : public analyzer{
    ...
    virtual void analyze(TTree* in, TTree* cut = 0) override{
        ...
        if(!( f->GetKey(name.c_str()))){ // skip if plot already done for given file
            ...
            std::vector<float> *isLooseMuon_ = 0;  in->SetBranchAddress("isLooseMuon", &isLooseMuon_);
            std::vector<float> *isTightMuon_ = 0;  in->SetBranchAddress("isTightMuon", &isTightMuon_);
            std::vector<float> *isHighPtMuon_ = 0;  in->SetBranchAddress("isHighPtMuon", &isHighPtMuon_);
            ...
            if(cut){
                ...
                for(int i = 0; i < paircount; i++){
                    if (ci & one){
                        ...
                        q_1 = (*tauq_)[i];
                        mt_1 = (*taumt_)[i];
                        againstElectronLooseMVA5_1 = (*againstElectronLooseMVA5_)[i];
                        againstElectronMediumMVA5_1 = (*againstElectronMediumMVA5_)[i];
                        againstElectronTightMVA5_1 = (*againstElectronTightMVA5_)[i];
                        againstElectronVLooseMVA5_1 = (*againstElectronVLooseMVA5_)[i];
                        againstElectronVTightMVA5_1 = (*againstElectronVTightMVA5_)[i];
                        againstMuonLoose3_1 = (*againstMuonLoose3_)[i];
                        againstMuonTight3_1 = (*againstMuonTight3_)[i];
                        byCombinedIsolationDeltaBetaCorrRaw3Hits_1 = (*byCombinedIsolationDeltaBetaCorrRaw3Hits_)[i];
                        byIsolationMVA3newDMwoLTraw_1 = (*byIsolationMVA3newDMwoLTraw_)[i];
                        byIsolationMVA3oldDMwoLTraw_1 = (*byIsolationMVA3oldDMwoLTraw_)[i];
                        byIsolationMVA3newDMwLTraw_1 = (*byIsolationMVA3newDMwLTraw_)[i];
                        byIsolationMVA3oldDMwLTraw_1 = (*byIsolationMVA3oldDMwLTraw_)[i];
                        chargedIsoPtSum_1 = (*chargedIsoPtSum_)[i];
                        decayModeFinding_1 = (*decayModeFinding_)[i];
                        decayModeFindingNewDMs_1 = (*decayModeFindingNewDMs_)[i];
                        neutralIsoPtSum_1 = (*neutralIsoPtSum_)[i];
                        puCorrPtSum_1 = (*puCorrPtSum_)[i];
